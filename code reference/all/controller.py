...
class RoadsideController:
    def __init__(self, env, communication_delay_mean=0.025, communication_delay_std=0.012):
    ...
    def update(self):
        ...
        # 3. 按预计到达汇入点时间排序
        all_vehicles = mainline_vehicles + ramp_vehicles
        ...
        # 4. 轨迹/速度/间距规划
        self.planned_speeds, self.planned_gaps = self.planner.plan_trajectory(self.merging_sequence, self.env)

        # 5. 控制执行
        self.executor.apply_control(self.planned_speeds, self.planned_gaps, self.actual_speeds)
